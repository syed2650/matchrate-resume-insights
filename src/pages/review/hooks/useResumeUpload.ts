...
const PDF_UPLOAD_TEMPORARILY_DISABLED_MESSAGE =
  "PDF uploads are temporarily disabled while we fix parsing reliability. Please upload a DOCX, TXT, or image file, or paste your resume text directly.";
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      if (file.type.startsWith('image/') || file.type === 'text/plain') {
        ...
        setHasParsingError(true);
        throw new Error(PDF_UPLOAD_TEMPORARILY_DISABLED_MESSAGE);
        ...
        try {
          ...
          if (extractedText.trim().length < 50) {
            setHasParsingError(true);
             throw new Error("Unsupported file format. Please upload a DOCX, TXT file, or an image.");
          }
        } catch (error) {
          setHasParsingError(true);
           throw new Error("Unsupported file format. Please upload a DOCX, TXT file, or an image.");
      ...
      toast({
        ...
        description: error instanceof Error 
          ? error.message 
            : "We couldn't extract text from your resume. Please upload a DOCX, TXT, or image file, or copy-paste your resume text manually.",
        variant: "destructive"
      });
    } finally {
      setIsParsingResume(false);
