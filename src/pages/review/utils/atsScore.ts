...
export function calculateATSScore(resumeText: string, jobDescriptionText: string): number {
  ...
  try {
    ...
    const baseScore = totalKeywords > 0 ? Math.round((matches / totalKeywords) * 100) : 50; // Default to 50 if no keywords found
    ...
    // Final score with bonuses, capped between 30-99
    let finalScore = Math.min(99, Math.max(30, baseScore + bonusPoints));
    ...
    console.log(`ATS Score calculated: ${finalScore}, Base: ${baseScore}, Bonus: ${bonusPoints}`);
...
export function getATSScoreExplanation(score: number): string {
  if (score >= 90) {
    ...
    return "This resume could use optimization for ATS systems. Consider improving keyword matching and section formatting.";
  } else {
...
export function getATSScoreDetail(score: number): string {
  if (score >= 90) {
    ...
    return "Your resume may struggle to be properly ranked by ATS systems. Consider adding more relevant keywords and improving formatting.";
  } else {
