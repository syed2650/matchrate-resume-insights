...
import { FileText, Link2, FileUp, Loader2, AlertCircle } from "lucide-react";
...
import { ExtractionStatus, ResumeFile } from "./types";
  ...
  const extractTextFromFile = async (file: File): Promise<string> => {
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (event) => {
        try {
          if (event.target?.result) {
            // Convert ArrayBuffer to text if binary file
      ...
      if (file.type === 'application/pdf') {
        // For PDF files, we'd ideally use PDF.js or similar, but for simplicity:
        reader.readAsArrayBuffer(file);
      } else {
        // For text files or Word docs (converted to text)
